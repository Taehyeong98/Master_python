...
twist_movement_2 = np.zeros((goal_time_joint,len(start_pose_rad)))
twist_movement_3 = np.zeros((goal_time_joint,len(start_pose_rad)))
for j in range(goal_time_joint):
    twist_movement_2[j]= twist_start_2 + twist_step * j
    twist_movement_3[j] = twist_start_3 + twist_step * j

goal_time = goal_time_joint*n_joints+add_time
...
for i in range(n_joints):
    ...
    step = goal_distance / (goal_time_joint - 1)

    for j in range(goal_time_joint):
        goal_movement_joint[j, i] = start + step * j
...
print(allConfigTraj)
#allConfigTraj =np.rstack((linear_movment))
